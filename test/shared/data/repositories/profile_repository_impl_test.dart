...
void main() {
  ...
  setUpAll(() {
    registerFallbackValue(const Profile(id: 'dummy', fullName: 'dummy', email: 'dummy'));
  });
  ...
  group('getCurrentProfile', () {
    ...
    const mockProfile = Profile(id: userId, fullName: 'John Doe', email: 'john@doe.com');

    test('retorna el perfil remoto y lo guarda localmente cuando está conectado y la llamada remota tiene éxito', () async {
      // Arrange
      ...
      when(() => mockLocalService.getById(userId)).thenAnswer((_) async => null);
      when(() => mockRemoteService.getProfileById(userId)).thenAnswer((_) async => mockDto);
    ...
    test('retorna el perfil local cuando está conectado pero la llamada remota falla', () async {
      // Arrange
      when(() => mockNetworkChecker.isConnected).thenAnswer((_) async => true);
      when(() => mockLocalService.getById(userId)).thenAnswer((_) async => mockProfile);
      when(() => mockRemoteService.getProfileById(userId)).thenThrow(Exception('Network Error'));
    ...
    test('retorna el perfil local cuando está conectado pero la llamada remota retorna nulo', () async {
      // Arrange
      when(() => mockNetworkChecker.isConnected).thenAnswer((_) async => true);
      when(() => mockLocalService.getById(userId)).thenAnswer((_) async => mockProfile);
      when(() => mockRemoteService.getProfileById(userId)).thenAnswer((_) async => null);
    ...
    test('lanza ProfileNotFoundException cuando está desconectado y no hay perfil local', () async {
      // Arrange
      when(() => mockNetworkChecker.isConnected).thenAnswer((_) async => false);
      when(() => mockLocalService.getById(userId)).thenAnswer((_) async => null);
    ...
    test('lanza ProfileNotFoundException cuando está conectado, la llamada remota falla y no hay perfil local', () async {
      // Arrange
      when(() => mockNetworkChecker.isConnected).thenAnswer((_) async => true);
      when(() => mockLocalService.getById(userId)).thenAnswer((_) async => null);
      when(() => mockRemoteService.getProfileById(userId)).thenThrow(Exception('Network error'));
